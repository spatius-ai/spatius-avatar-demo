...
export default function ControlPanel({ activeAvatar, activeController, multiMode, avatarSlots, activeUid, onSlotSelect }: Props) {
  ...
  const handleHostMessage = useCallback((msg: any) => {
    ...
    if (type === 'ready') {
      ...
      const avatarId = activeAvatar?.characterId || ''
  ...
  }, [activeController, backendConnecting, handleHostMessage])
      ...
      {activeAvatar && (
        <div className="status-bar">
          <div className="status-row">
            ...
            <span className="status-value">{backendConnected ? 'connected' : backendConnecting ? 'connecting...' : 'disconnected'}</span>
          </div>
          <div className="status-row">
